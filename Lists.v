...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => nil
  | n :: t => if even n then oddmembers t
              else n :: oddmembers t
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | n :: t => if n =? v then remove_all v t
...
Theorem bag_count_sum: 
  forall (n : nat), 
  forall (s1 s2: bag),
  (count n s1) + (count n s2) = count n (s1 ++ s2).
Proof.
  intros n s1 s2.
  induction s1 as [| sx s1' IHs1'].
  - reflexivity.
  - simpl. destruct (sx =? n).
